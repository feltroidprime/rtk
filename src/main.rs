...
fn main() -> Result<()> {
    ...
    match cli.command {
        Commands::Rtk(cmd) => match cmd {
            RtkCommands::CcEconomics {
                daily,
                weekly,
                monthly,
                all,
        ...
        Commands::Filters(cmd) => match cmd {
            FilterCommands::Cargo { command } => match command {
            ...
            FilterCommands::Curl { args } => {
                curl_cmd::run(&args, cli.verbose)?;
            }

            FilterCommands::Deps { path } => {
                deps::run(&path, cli.verbose)?;
            }

            FilterCommands::Diff { file1, file2 } => {
            ...
            FilterCommands::Docker { command } => match command {
                DockerCommands::Ps => {
            ...
            FilterCommands::Env { filter, show_all } => {
                env_cmd::run(filter.as_deref(), show_all, cli.verbose)?;
            }

            FilterCommands::Err { command } => {
            ...
            FilterCommands::Format { args } => {
                format_cmd::run(&args, cli.verbose)?;
            }

            FilterCommands::Gh { subcommand, args } => {
                gh_cmd::run(&subcommand, &args, cli.verbose, cli.ultra_compact)?;
            }

            FilterCommands::Git { command } => match command {
            ...
            FilterCommands::Go { command } => match command {
            ...
            FilterCommands::GolangciLint { args } => {
            ...
            FilterCommands::Json { file, depth } => {
                if file == Path::new("-") {
            ...
            FilterCommands::Kubectl { command } => match command {
            ...
            FilterCommands::Lint { args } => {
                lint_cmd::run(&args, cli.verbose)?;
            }

            FilterCommands::Log { file } => {
                if let Some(f) = file {
            ...
            FilterCommands::Ls { args } => {
                ls::run(&args, cli.verbose)?;
            }

            FilterCommands::Next { args } => {
                next_cmd::run(&args, cli.verbose)?;
            }

            FilterCommands::Npm { args } => {
                npm_cmd::run(&args, cli.verbose, cli.skip_env)?;
            }

            FilterCommands::Npx { args } => {
                if args.is_empty() {
                        ...
                        if args.len() > 1 {
                            ...
                            match args[1].as_str() {
                                ...
                                _ => {
                                    ...
                                    let status =
                                        cmd.status().context("Failed to run npx prisma")?;
            ...
            FilterCommands::Pip { args } => {
                pip_cmd::run(&args, cli.verbose)?;
            }

            FilterCommands::Playwright { args } => {
                playwright_cmd::run(&args, cli.verbose)?;
            }

            FilterCommands::Pnpm { command } => match command {
            ...
            FilterCommands::Prettier { args } => {
                prettier_cmd::run(&args, cli.verbose)?;
            }

            FilterCommands::Prisma { command } => match command {
            ...
            FilterCommands::Ruff { args } => {
                ruff_cmd::run(&args, cli.verbose)?;
            }

            FilterCommands::Scarb { command } => match command {
            ...
            FilterCommands::Snforge { command } => match command {
            ...
            FilterCommands::Summary { command } => {
                let cmd = command.join(" ");
                summary::run(&cmd, cli.verbose)?;
            }

            FilterCommands::Test { command } => {
                let cmd = command.join(" ");
                runner::run_test(&cmd, cli.verbose)?;
            }

            FilterCommands::Tree { args } => {
                tree::run(&args, cli.verbose)?;
            }

            FilterCommands::Tsc { args } => {
                tsc_cmd::run(&args, cli.verbose)?;
            }

            FilterCommands::Vitest { command } => match command {
                VitestCommands::Run { args } => {
                    vitest_cmd::run(vitest_cmd::VitestCommand::Run, &args, cli.verbose)?;
                }
            },

            FilterCommands::Wc { args } => {
                wc_cmd::run(&args, cli.verbose)?;
            }

            FilterCommands::Wget { url, stdout, args } => {
                if stdout {
